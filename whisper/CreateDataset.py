...
for number in range(2,41,1):
    fnames = []
    for i in range (50):
        fname = "./samples/" + str(number) + "/" + str(number) + "_" + str(number) + ".wav"
        if os.path.exists(fname):
            fnames.append(fname)
    fname = "./samples/" + str(number) + "/" + str(number) + "_50.wav"
    if os.path.exists(fname):
...
audio_dataset.push_to_hub("SameerMahajan/marathi_numbers-1-40")
